  ...
  const PRESETS={
    ...
    macro:[
      ...
      {id:'selective',title:'嚴格確認',intent:'要求更深回撤與更明顯的波動擴張，只留下較少候選。',config:{macro:{p:{
        '1h':{dd:.14,atrX:1.30,confirm:18},'4h':{dd:.20,atrX:1.30,confirm:14},'1d':{dd:.28,atrX:1.30,confirm:9},
        '1w':{dd:.38,atrX:1.22,confirm:5},'1m':{dd:.38,atrX:1.15,confirm:3}
      }}}}
    ],
    sweep:[
    ...
    basin:[
      {id:'sensitive',title:'短期整理',intent:'較短的平盤也能被研究，門檻較寬鬆，候選通常較多。',config:{exp:{basin:{floorMin:6,floorMax:50,cliffMax:40,threshold:60}}}},
      {id:'balanced',title:'平衡盆地',intent:'兼顧短期與較完整的下山後走平結構。',config:{exp:{basin:{floorMin:10,floorMax:80,cliffMax:60,threshold:68}}}},
      {id:'selective',title:'大型盆地',intent:'忽略短暫整理，要求更長平盤與更高盆地分數。',config:{exp:{basin:{floorMin:20,floorMax:140,cliffMax:100,threshold:76}}}}
    ],
    field:[
    ...
    echo:[
      ...
      {id:'balanced',title:'平衡回聲',intent:'用 32 根 K 線與 82% 強回聲門檻作為研究起點。',config:{exp:{echo:{length:32,history:1000,similarity:.82,followBars:12}}}},
      {id:'selective',title:'長型態・高相似',intent:'用更長片段與更高相似門檻，只留下較接近的強回聲。',config:{exp:{echo:{length:48,history:1500,similarity:.90,followBars:12}}}}
    ],
    astro:[
