...
1. **M3.5 Research Runner**：把模型運算抽成 UI / Runner 共用引擎，建立 headless 批次研究能力，讓跨市場、週期、參數與模型版本比較不需要人工操作 Lab。
2. **M4「一眼懂，點下去很深」**：把同一份 Evidence 與 Evaluation 透過嚴格 UI 入場條件做成漸進式理解路徑，不建立新手／專業模式。

Runner 是研究基礎設施，不另做一套模型，也不以大型永久資料庫作為前置條件。
