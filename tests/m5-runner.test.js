...
const queries=[
  ...
  {
    name:'support candidate + basin forming price overlap',
    conditions:[
...
assert.equal(run.conditional.length,2);
...
assert(x.baseline.episodeCount>=x.episodeCount,'adding conditions must not create more samples than anchor episodes');
...
const priceResult=run.conditional[1].result;
assert.equal(priceResult.conditions[1].relation,'price_overlap');
const priceDirect=CE.evaluateConditions(data,run.evidence,queries[1].conditions,{horizons:[3,5,10,20],knownThrough:data.length-1});
assert.deepStrictEqual(priceResult,priceDirect,'Runner price relation must use the exact shared Condition Engine result');
assert(priceResult.episodeCount<=priceResult.baseline.episodeCount,'price relation cannot create more samples than the anchor');
